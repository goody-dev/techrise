...
export default function Faq() {
  ...
  return (
    <div className="w-100vw max-w-[100%] h-[100vh] flex flex-row items-center justify-center p-2">
      ...
      <div className="w-[100%] max-w-[450px] flex flex-col items-center justify-center gap-2 p-3 rounded-md shadow-md text-black bg-white">
          ...
          {faqs.map((faq) => 
            <li key={faq.id} className="w-[100%] flex flex-col p-1">
              <div className="w-[100%] flex flex-row items-start justify-between gap-1">
                <p>{faq.question}</p>
                <span onClick={() => handleExpand(faq.id)} className="cursor-pointer">{openedQuestion === faq.id? "-": "+"}</span>
              </div>
              {faq.id === openedQuestion && <p>{faq.answer}</p>}
            </li>
          )}
        </ul>
      </div>
